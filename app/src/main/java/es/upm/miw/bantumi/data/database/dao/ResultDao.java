...
public interface ResultDao {
    ...
    @Query("SELECT * FROM results " +
            "ORDER BY MAX(seedsPlayer1, seedsPlayer2) DESC, id ASC " +
            "LIMIT 10")
    List<ResultEntity> getTop10ByBestSeedsList();

    // DESC: filtros opcionales (null-safe)
    @Query("SELECT * FROM results " +
    ...
    // ASC: filtros opcionales (null-safe)
    @Query("SELECT * FROM results " +
            "WHERE (:wins IS NULL OR player1Won = :wins) " +
            "AND (:mode IS NULL OR mode = :mode) " +
            "AND (:name IS NULL OR LOWER(player1Name) LIKE '%' || LOWER(:name) || '%') " +
            "ORDER BY MAX(seedsPlayer1, seedsPlayer2) ASC, id ASC " +
            "LIMIT 10")
